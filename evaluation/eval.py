...
try:
    from openai import OpenAI # New OpenAI library
...
def evaluate_popqa_jsonl(predictions_jsonl, golden_jsonl):
    ...
    # Create question-based matching instead of sequential
    golden_dict = {}
    for gold_item in golden_jsonl:
        question = gold_item.get('question', gold_item.get('query', ''))
        if question:
            norm_question = normalize_answer(question)
            golden_dict[norm_question] = gold_item
    
    print(f"Built golden dictionary with {len(golden_dict)} unique normalized questions")
    
    matched_pairs = []
    unmatched_predictions = []
    
    for pred_item in predictions_jsonl:
        question = pred_item.get('query', pred_item.get('question', ''))
        if question:
            norm_question = normalize_answer(question)
            if norm_question in golden_dict:
                matched_pairs.append((pred_item, golden_dict[norm_question]))
            else:
                unmatched_predictions.append(pred_item)
                if len(unmatched_predictions) <= 3:  # Show first few unmatched
                    print(f"Unmatched: {question[:60]}... -> {norm_question[:60]}...")
    
    print(f"Matched {len(matched_pairs)} prediction-golden pairs based on questions")
    print(f"Found {len(unmatched_predictions)} unmatched predictions")
    
    if len(matched_pairs) == 0:
        print("No matched pairs found for evaluation.")
        return {"accuracy_loose_match": 0.0, "evaluated_count": 0, "total_predictions": len(predictions_jsonl)}

    print(f"Evaluating {len(matched_pairs)} matched pairs for POPQA using Loose Match...")
    for pred_item, gold_item in tqdm(matched_pairs, desc="Evaluating POPQA (Loose Match)"):
        generated_answer = pred_item.get("generated_answer", pred_item.get("processed_answer", ""))
    ...
    results = {
        "accuracy_loose_match": accuracy, 
        "evaluated_count": evaluated_count, 
        "total_predictions": len(predictions_jsonl),
...
def evaluate_pubqa_jsonl(predictions_jsonl, golden_jsonl):
    ...
    if len(matched_pairs) == 0:
        print("No matched pairs found for evaluation.")
        return {"accuracy": 0.0, "evaluated_count": 0, "total_predictions": len(predictions_jsonl)}
    ...
    for pred_item, gold_item in tqdm(matched_pairs, desc="Evaluating PUBQA"):
        ...
        if "answer" in gold_item:
            # 格式1: {"answer": "true"}
            golden_answer = str(gold_item.get("answer", "")).lower().strip()
        elif "answers" in gold_item:
            # 格式2: {"answers": ["true"]}
            answers_list = gold_item.get("answers", [])
            if answers_list and len(answers_list) > 0:
                golden_answer = str(answers_list[0]).lower().strip()
...
def evaluate_arc_challenge_jsonl(predictions_jsonl, golden_jsonl):
    ...
    if len(matched_pairs) == 0:
        print("No ARC data to evaluate after matching.")
        return {"accuracy": 0.0, "evaluated_count": 0, "total_predictions": len(predictions_jsonl)}

    print(f"Evaluating {len(matched_pairs)} matched pairs for ARC Challenge...")
    for pred_item, gold_item in tqdm(matched_pairs, desc="Evaluating ARC Challenge"):
        generated_answer_raw = pred_item.get("generated_answer", pred_item.get("processed_answer", ""))
...
def initialize_openai_client(api_key, base_url=None, timeout=60):
    ...
    if OPENAI_NEW_VERSION:
        ...
        # Old OpenAI library
        print(f"Using old OpenAI API (version < 1.0)")
        openai.api_key = api_key
        if base_url:
            openai.api_base = base_url
            print(f"Set OpenAI API base to: {base_url}")
        return None  # No client object in old version
...
def evaluate_file_jsonl(golden_filepath, answer_filepath, dataset_type, num_samples=-1, 
    ...
    # Don't align/truncate data for question-based matching
    # We want to use ALL golden data for matching
    
    if dataset_type == 'POPQA':
        return evaluate_popqa_jsonl(answer_data_list, golden_data_list)
    elif dataset_type == 'PUBQA':
        return evaluate_pubqa_jsonl(answer_data_list, golden_data_list)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, required=True, choices=['POPQA', 'PUBQA', 'BIO', 'ARC_Challenge'], help="Type of dataset to evaluate.")
